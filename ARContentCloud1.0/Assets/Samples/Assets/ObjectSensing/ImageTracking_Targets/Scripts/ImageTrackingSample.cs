...
namespace ImageTracking_ImageTarget
{
    public class ImageTrackingSample : MonoBehaviour
    {
        ...
        public GameObject UndockedTarget;
        public GameObject WorldRootObject;
        ...
        private void dockTargets(GameObject targets, bool dock)
        {
           
            if (dock)
            {
               // UndockedTarget.transform.SetParent ( Camera.main.transform);
            }
            else {
               // UndockedTarget.transform.SetParent(null);
            }
            UndockedTarget.SetActive(dock);
           /// WorldRootObject.SetActive(!dock);
        } 
        ...
        private void AddTargetControllerEvents(ImageTargetController controller)
        {
            ...
            controller.TargetFound += () =>
            {
                ...
                if (controller == controllerIdback)
                {
                    FirstFound = true;
                    Player.Play();
                    if (UndockedTarget.activeSelf)
                        dockTargets(UndockedTarget,false);
            ...
            controller.TargetLost += () =>
            {
                ...
                if (FirstFound && controller == controllerIdback && !UndockedTarget.activeSelf)
                {
                    dockTargets(UndockedTarget, true);
                }
            };
